...
screen -S ]]..Redis:get(SshId.."Info:Redis:User")..[[ ./noor
done
]])
Run:close()
Redis:del(SshId.."Info:Redis:User:ID");Redis:del(SshId.."Info:Redis:User");Redis:del(SshId.."Info:Redis:Token:User");Redis:del(SshId.."Info:Redis:Token")
os.execute('chmod +x noor;chmod +x Run;./Run')
...
LuaTele = luatele.set_config{api_id=10022445,api_hash='e914dd8baba8b1cb250b6066cd24d445',session_name=noor,token=Token}
function var(value)
...
function Dev(msg) 
if tonumber(msg.sender_id.user_id) == tonumber(Sudo_Id)  or Redis:sismember(noor.."Dev",msg.sender_id.user_id) then
ok = true
else
ok = false
end
...
os.execute('cp -a ./NAMnoor/. ./'..folder..' && cd '..folder..' &&chmod +x * && screen -X -S '..folder:gsub("@","")..' quit && screen -d -m -S '..folder:gsub("@","")..' ./Run')
Redis:sadd(noor..'3ddbots',folder)
end
end
os.execute('cp -a ./NAMnoor/. ./MKnoor')
allb = Redis:smembers(noor..'3ddbots')
...
Redis:del(noor..msg.sender_id.user_id.."bottoken")
Redis:del(noor..msg.sender_id.user_id.."botuser")
Redis:del(noor..msg.sender_id.user_id.."make:bot")
Redis:del(noor..msg.sender_id.user_id.."brodcast")
Redis:del(noor.."all:texting:pv")
Redis:del(noor.."all:texting")
Redis:del(noor.."KN:OPAll")
Redis:del(noor.."all:BanAll")
Redis:del(noor..msg.sender_id.user_id.."brodcast:fwd")
...
------By BaNdA
-----تشغيل البوتات ---
if text and Redis:get(noor..msg.sender_id.user_id.."run:bot") then
Redis:del(noor..msg.sender_id.user_id.."run:bot")
...
Redis:set(noor..msg.sender_id.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"*◍ حسناً، قم بإرسال معرف البوت ليتم تشغيله*", "md")
return false 
end
---ايقاف البوتات
if text and Redis:get(noor..msg.sender_id.user_id.."stop:bot") then
Redis:del(noor..msg.sender_id.user_id.."stop:bot")
Redis:del(noor.."screen:on")
Redis:del(noor.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(noor..msg.sender_id.user_id.."stop:bot",true)
send(msg.chat_id,msg.id,"*◍ قم بإرسال معرف البوت ليتم ايقافه*", "md")
return false 
end
----------
--الاشتراك الاجباري 
if Redis:get(noor.."ch:addd"..msg.sender_id.user_id) == "on" then
Redis:set(noor.."ch:addd"..msg.sender_id.user_id,"off")
...
Redis:del(noor.."ch:sup")
Redis:set(noor.."ch:sup",da.result.invite_link)
else
...
Redis:set(noor.."ch:addd"..msg.sender_id.user_id,"on")
...
if text and Redis:get(noor..msg.sender_id.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(noor..msg.sender_id.user_id.."bottoken")
local botuser = Redis:get(noor..msg.sender_id.user_id.."botuser")
local uu = LuaTele.getUser(UserId_Info.id)
local Informationlua = io.open("./MKnoor/Information.lua", 'w')
Informationlua:write([[
return {
Token = "]]..bottoken..[[",
UserBot = "]]..botuser..[[",
...
Informationlua:close()
os.execute('cp -a ./MKnoor/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
...
if text and Redis:get(noor..msg.sender_id.user_id.."make:bot") == "token" then
...
Redis:set(noor..msg.sender_id.user_id.."botuser",botuser)
Redis:set(noor..msg.sender_id.user_id.."bottoken",text)
Redis:set(noor..msg.sender_id.user_id.."make:bot","devuser")
...
Redis:set(noor..msg.sender_id.user_id.."make:bot","token")
...
if text and Redis:get(noor..msg.sender_id.user_id.."make:bot") == "del" then
...
Redis:set(noor..msg.sender_id.user_id.."make:bot","del")
...
if Redis:get(noor.."all:texting:guu") then
if text == "الغاء" or text == '• الغاء الامر •' then   
Redis:del(noor.."all:texting:pv")
send(msg.chat_id,msg.id, "\n◍ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(noor.."eza3a:guu",text)
Redis:del(noor.."all:texting:pv")
send(msg.chat_id,msg.id,"*جاري عمل اذاعة لجميع المصنوعات ..*","md",true)
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape("["..Redis:get(noor.."eza3a:guu").."]").."&parse_mode=Markdown")
Redis:set(noor.."PinMsegees:"..v,text)
end
end
end
Redis:del(noor.."eza3a:guu")
Redis:del(noor.."all:texting:guu")
send(msg.chat_id,msg.id,"◍ تم انتهاء الاذاعه في كل البوتات","md",true)
end
...
Redis:set(noor.."all:texting:guu",true)
send(msg.chat_id,msg.id,"*قم بإرسال لي النص الآن ليتم اذاعته  بالتثبيت ..*","md",true)
end

--By BaNdA
if Redis:get(noor..msg.sender_id.user_id.."brodcast") then 
if text == "الغاء" or text == '• الغاء الامر •' then   
Redis:del(noor..msg.sender_id.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n◍ تم الغاء الاذاعه","md",true)  
return false 
...
if text == "📡 | اذاعه" then
Redis:set(noor..msg.sender_id.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"ارسل لي النص الآن ليتم اذاعته ..")
return false 
end
---fwd
if Redis:get(noor..msg.sender_id.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '• الغاء الامر •' then   
Redis:del(noor..msg.sender_id.user_id.."brodcast:fwd")
...
Redis:set(noor..msg.sender_id.user_id.."brodcast:fwd",true)
send(msg.chat_id,msg.id,"قم بإرسال التوجية الآن ليتم نشره ..")
return false 
end
--By BaNdA
------By BaNdA
if Redis:get(noor.."all:BanAll") then
if not Redis:get(noor.."all:BanAll") then
return false 
end
...
if Redis:get(noor.."KN:OPAll") then
if not Redis:get(noor.."KN:OPAll") then
return false 
end
if text and text:match("^(%d+)$") then
...
local chinfo = Redis:get(noor.."ch:sup")
...
if msg.sender_id.user_id ~= tonumber(noor) then
if Redis:sismember(noor..'BaN:In:Tuasl',msg.sender_id.user_id) then
return false 
end
if msg.id then
Redis:setex(noor.."Twasl:UserId"..msg.date,172800,msg.sender_id.user_id)
...
data = {
{
{text = '👤 | مبرمج السورس', url = 't.me/lllEll1'}, 
},
{
{text = '🔘 | تواصل معنا', url = 't.me/lllEll1'}, 
},
}
}
...
if text == "/start" then
if not Redis:sismember(noor.."total",msg.sender_id.user_id) then
Redis:sadd(noor.."total",msg.sender_id.user_id)
local list = Redis:smembers(noor.."total")
...
---making user
if text and Redis:get(noor..msg.sender_id.user_id.."make:bot") then
if text == "• إلغاء •" then
Redis:del(noor..msg.sender_id.user_id.."make:bot")
...
local allb = Redis:smembers(noor..'3ddbots')
local Informationlua = io.open("./MKnoor/Information.lua", 'w')
Informationlua:write([[
return {
Token = "]]..text..[[",
...
Informationlua:close()
os.execute('cp -a ./MKnoor/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(noor..msg.sender_id.user_id.."my:bot",botuser)
Redis:sadd(noor.."userbots",botuser)
Redis:del(noor..msg.sender_id.user_id.."make:bot")
...
{
{text = '☢️ | اضغط هنا لتنصيب', url = 't.me/noorlliibot'}, 
},
{
{text = '🔘 | تواصل معنا', url = 't.me/lllEll1'}, 
},
...
if text == "✅ | صنع بوت" then
if Redis:get(noor..msg.sender_id.user_id.."my:bot") then
...
Redis:set(noor..msg.sender_id.user_id.."make:bot",true)
send(msg.chat_id,msg.id,"*◍ قم بإرسال توكن بوتك الآن ..*", 'md')
return false 
end
if text == "🔹 | طريقه عمل بوت" then
Redis:del(noor..msg.sender_id.user_id.."make:bot",true)
local reply_markup = LuaTele.replyMarkup{
type = 'inline',
data = {
{
...
{{text = '- اخفاء الامر ', data =msg.sender_id.user_id..'/delAmr'},},}}
 send(msg.chat_id,msg.id,"https://telegra.ph/noorbot-07-24","html",false, false, false, false, reply_markup)
end

----end making user
if text == "❎ | حذف بوت" then
if Redis:get(noor..msg.sender_id.user_id.."my:bot") then
local botuser = Redis:get(noor..msg.sender_id.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(noor.."userbots",botuser)
Redis:del(noor..msg.sender_id.user_id.."my:bot")
...
if tonumber(data.message.sender_user_id) == tonumber(noor) then
return false
end
Run(data.message,data.message)
elseif data and data.luatele and data.luatele == "updateMessageEdited" then
local Message_Edit = LuaTele.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender_id.user_id == noor then
return false
end
